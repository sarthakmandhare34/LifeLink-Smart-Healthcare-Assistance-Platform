...
const SPECIALTY_MATCHES: Array<{ pattern: RegExp; specialty: string }> = [
  ...
  { pattern: /heart|chest|palpitation|blood pressure/i, specialty: 'Cardiology' },
  { pattern: /cough|breath|asthma|lung/i, specialty: 'Pulmonology' },
  { pattern: /stomach|abdominal|nausea|vomit|diarrh/i, specialty: 'Gastroenterology' },
...
function recommendSpecialty(summary: string) {
  return SPECIALTY_MATCHES.find(({ pattern }) => pattern.test(summary))?.specialty ?? 'General Practice';
}
...
export const analyzeSymptoms = async (payload: AssessmentPayload): Promise<AssessmentResponse> => {
  ...
  if (hasPattern(summary, RED_FLAG_PATTERNS)) {
    return {
      ...
      reason: 'This local decision-support check detected symptom wording that can be associated with an emergency. It cannot assess severity or make a diagnosis.',
  ...
  const needsPromptReview = hasPattern(summary, PROMPT_REVIEW_PATTERNS) || isLongDuration || (hasKnownCondition && payload.duration !== '< 24 hours') || payload.age >= 65;

  if (needsPromptReview) {
    return {
      urgency: 'MODERATE',
      recommendedSpecialty: specialty,
      reason: 'This local decision-support check suggests a clinician should review the reported symptoms soon. It does not identify a condition or replace professional assessment.',
      guidance: 'Arrange a timely consultation with a licensed clinician. Seek urgent help sooner if symptoms worsen, new red-flag symptoms appear, or you feel unsafe.',
    };
  }
  ...
  return {
    urgency: 'LOW',
    recommendedSpecialty: specialty,
    reason: 'No emergency-related terms were detected by the local decision-support check. This is not a diagnosis and cannot rule out a health problem.',
    guidance: 'Monitor how you feel and consider contacting a licensed clinician if symptoms persist, worsen, or you are concerned.',
  };
};
