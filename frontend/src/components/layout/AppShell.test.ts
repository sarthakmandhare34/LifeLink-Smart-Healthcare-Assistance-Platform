...
import {
  LIFELINK_FULL_LOGO_PRESENTATION_CLASS,
  LIFELINK_OFFICIAL_LOGO_SYMBOL_URL,
  LIFELINK_OFFICIAL_LOGO_URL,
} from "../brand/LifeLinkLogo";
...
describe("patient sidebar branding", () => {
  ...
  it("uses self-contained local LifeLink vector branding instead of managed storage assets", () => {
    expect(LIFELINK_OFFICIAL_LOGO_URL).toBe("local-vector-wordmark");
    expect(LIFELINK_OFFICIAL_LOGO_SYMBOL_URL).toBe("local-vector-symbol");
  });
