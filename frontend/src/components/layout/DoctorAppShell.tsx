...
export const DoctorAppShell = () => {
  ...
  if (session.isLoading) return <main className="doctor-content"><div className="container"><p>Verifying synthetic doctor session…</p></div></main>;
  if (!session.data) return <Navigate to="/doctor/login" replace />;

  return (
    <div className="doctor-layout">
      <aside className="doctor-sidebar">
        <div className="doctor-sidebar-brand"><LifeLinkLogo className="lifelink-logo-sidebar" /></div>
        <nav className="doctor-sidebar-nav">
          {navItems.map(({ path, label, icon: Icon }) => <NavLink key={path} to={path} className={({ isActive }) => `doctor-nav-item ${isActive ? "active" : ""}`}><Icon size={19} /><span>{label}</span></NavLink>)}
        </nav>
      </aside>
      <div className="doctor-main">
        <header className="doctor-header">
          ...
          <NavLink to="/doctor/profile" className="doctor-profile-link">
            ...
            <span>{session.data.displayName}</span><User size={18} />
          </NavLink>
        </header>
        <main className="doctor-content"><div className="container"><Outlet /></div></main>
      </div>
    </div>
  );
};
