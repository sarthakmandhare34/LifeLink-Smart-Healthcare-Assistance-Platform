...
const selectorSource = readFileSync(
  resolve(process.cwd(), "frontend/src/features/entry/WorkspaceSelector.tsx"),
  "utf8"
);
const appSource = readFileSync(
  resolve(process.cwd(), "frontend/src/App.tsx"),
  "utf8"
);
...
describe("workspace selector entry flow", () => {
  it("keeps separate patient and doctor login destinations", () => {
    ...
    expect(selectorSource).toContain(
      'openWorkspace("clinician", "/doctor/login")'
    );
  ...
  it("keeps concise clinician workspace wording and a switch transition", () => {
    ...
    expect(selectorSource).toContain(
      "Review assigned appointments and patient context."
    );
  ...
  it("uses the clinical-portal header without unsafe clinical or emergency claims", () => {
    ...
    expect(selectorSource).not.toContain(
      "Emergency assistance is user-controlled."
    );
  ...
  it("keeps entry text and glass cards high contrast in dark mode", () => {
    expect(styleSource).toContain(
      ':root[data-theme="dark"] .workspace-choice-card'
    );
    expect(styleSource).toContain(
      ':root[data-theme="dark"] .workspace-entry-note'
    );
    expect(styleSource).toContain(
      ':root[data-theme="dark"] .workspace-entry-header h1'
    );
  });
});
