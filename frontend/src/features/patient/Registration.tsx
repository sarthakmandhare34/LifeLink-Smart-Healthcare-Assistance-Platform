    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Registration failed. Please verify your form details."
      );
      setIsLoading(false);
    }
  };
    ...
    <main className="auth-page" aria-labelledby="patient-registration-heading">
      ...
      <Card variant="glass" className="auth-card auth-card-wide">
        <header className="auth-card-header">
          ...
          <p>
            Your Health Passport details can be completed privately after
            account creation.
          </p>
        </header>
        ...
        <div
          ...
          aria-label="Alternative account creation methods"
        >
          ...
          <div className="social-auth-actions">
            <Button
              ...
              variant="outline"
              className="w-full"
              disabled={!providerQuery.data?.googleRegistrationStartUrl}
              onClick={() => {
          ...
          {!providerQuery.isLoading && providerQuery.data?.google && (
            <p className="caption social-auth-note">
              Create your LifeLink account with Google here, then use Google
              sign-in next time.
            </p>
          )}
          {!providerQuery.isLoading && !providerQuery.data?.google && (
            <p className="caption social-auth-note">
              Google account creation will activate once it is securely
              connected.
            </p>
          )}
        </div>
