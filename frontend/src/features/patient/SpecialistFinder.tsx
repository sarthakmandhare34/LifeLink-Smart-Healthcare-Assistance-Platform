...
export const BROWSER_LOCATION_PRIVACY =
  "Optional: use your browser location to order only the visible controlled specialist entries. Your location is not stored or sent to LifeLink.";
export const SPECIALTY_SEARCH_GUIDANCE =
  "Free-text search matches specialties only. Use the Mumbai area and station filters below for where you live.";
...
export const SPECIALIST_LOAD_ERROR_MESSAGE =
  "Please check your connection and try again. Your filters will stay unchanged.";
...
export const SpecialistFinder = () => {
  ...
  const discoveryFilters = useMemo(
    () => ({
      ...
      railLine:
        railLine === ALL_FILTER ? undefined : (railLine as MumbaiRailLine),
  ...
  const availableStations = useMemo(
    () =>
      (facetsQuery.data?.stations ?? []).filter(
        candidate =>
          railLine === ALL_FILTER ||
          candidate.lines.includes(railLine as MumbaiRailLine)
      ),
  ...
  useEffect(() => {
    if (
      station !== ALL_FILTER &&
      !availableStations.some(candidate => candidate.name === station)
    )
  ...
  const updateRailLine = (nextRailLine: string) => {
    ...
    if (
      station !== ALL_FILTER &&
      nextRailLine !== ALL_FILTER &&
      !availableStations
        .find(candidate => candidate.name === station)
        ?.lines.includes(nextRailLine as MumbaiRailLine)
    )
      setStation(ALL_FILTER);
  };
  ...
  const requestBrowserLocation = () => {
    if (!navigator.geolocation) {
      setLocationStatus(
        "This browser does not support location. You can still choose the Mumbai area closest to where you live."
      );
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      position => {
        ...
        setLocationStatus(
          "Location is active for this page only. Controlled specialist entries are ordered approximately from your browser location; no coordinates are stored or sent to LifeLink."
        );
        setIsLocating(false);
      },
      () => {
        setLocationStatus(
          "Location was not shared. You can continue using the Mumbai area and station filters; no location was stored."
        );
        setIsLocating(false);
      },
  ...
  const clearBrowserLocation = () => {
    ...
    setLocationStatus(
      "Browser location cleared. Directory results return to their standard controlled order; no location was stored."
    );
  };
  ...
  const handleRequest = async (doctorId: string, event: React.MouseEvent) => {
    ...
    if (!requestedAt) {
      setRequestError(
        "Choose a requested visit date and time before sending an appointment request."
      );
      return;
    }
    ...
    try {
      await requestMutation.mutateAsync({
        doctorId,
        scheduledAt: new Date(requestedAt),
  ...
  const displayedDoctors = useMemo(
    () =>
      browserLocation
        ? sortByBrowserLocation(filteredDoctors, browserLocation)
        : filteredDoctors,
    [browserLocation, filteredDoctors]
  );
  ...
  if (directoryQuery.isLoading || facetsQuery.isLoading)
    return (
      <div className="discovery-state" role="status" aria-live="polite">
        <RefreshCw
          size={20}
          className="discovery-state-icon"
          aria-hidden="true"
        />
        <p className="caption">
          Loading the controlled Mumbai specialist directory…
        </p>
      </div>
    );
  ...
  if (directoryQuery.isError || facetsQuery.isError)
    return (
      <div className="container" style={{ padding: 0 }}>
        <div role="alert">
          <Card variant="glass" className="discovery-load-error">
    ...
    <div className="container" style={{ padding: 0 }}>
      <header className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="caption">
            Browse controlled Mumbai specialist entries by specialty and the
            Mumbai area closest to where you live, then send a patient-owned
            appointment request.
          </p>
        </div>
      </header>
      ...
      <Card variant="glass" className="discovery-refinement-card mb-4">
        <div className="discovery-refinement-content">
          <div className="mock-directory-notice" role="note">
            ...
            <span>
              <strong>Controlled directory only.</strong> These are controlled
              specialist entries, not verified clinicians, live availability,
              ratings, or medical recommendations.
            </span>
          </div>
          <div className="discovery-search-field">
            ...
            <div style={{ flex: 1 }}>
              ...
              />
              <p className="caption discovery-search-guidance">
          ...
          <div className="discovery-location-control">
            <div>
              <p className="discovery-location-title">
                {BROWSER_LOCATION_TITLE}
              </p>
              <p className="caption discovery-location-copy" aria-live="polite">
                {locationStatus}
              </p>
            </div>
          ...
          <div className="discovery-filter-grid">
            ...
            <div>
              <label className="discovery-filter-label" htmlFor="rail-filter">
              ...
              <select
                ...
                className="discovery-filter-select"
                value={railLine}
                onChange={event => updateRailLine(event.target.value)}
              >
                <option value={ALL_FILTER}>Choose a rail corridor</option>
                {facets?.railLines.map(value => (
                  <option key={value} value={value}>
                    {value} line
                  </option>
                ))}
          ...
          <div className="discovery-request-grid">
            <div className="discovery-request-field">
              ...
              />
              <p className="caption">
                Choose a preferred time; availability is confirmed later.
              </p>
            </div>
          ...
          {requestError && (
            <div className="alert-panel">
              <span className="caption">{requestError}</span>
            </div>
          )}
        </div>
      </Card>
        ...
        <section>
          <div className="discovery-results-heading">
            <div>
              ...
              <p className="caption">
                {displayedDoctors.length} controlled{" "}
                {displayedDoctors.length === 1 ? "entry" : "entries"} match the
                current filters.
                {browserLocation
                  ? " They are ordered approximately from your browser location."
                  : ""}
              </p>
            </div>
                    ...
                    <div>
                      ...
                      <div className="flex-col gap-1 mt-2">
                        ...
                        <div className="caption flex items-center gap-1">
                          ...
                          • {doctor.station} station
                        </div>
                        <div className="caption flex items-center gap-1">
                          <TrainFront size={14} />{" "}
                          {doctor.railLines.join(" + ")} connectivity
                        </div>
                      </div>
                    </div>
                    <div
                    ...
                    >
                      {requestedDocId === doctor.id ? (
                        <Button variant="secondary" className="w-full" disabled>
                          Requested! Opening appointments…
                        </Button>
                      ) : (
                        <Button
                          variant="primary"
              ...
              <BentoItem colSpan={4}>
                ...
                >
                  <p className="text-muted" style={{ margin: 0 }}>
                    No controlled directory entries match the selected filters.
                    Clear a filter to view the available development entries.
                  </p>
                  {activeFilterCount > 0 && (
                    <Button
                      type="button"
          ...
          <Card variant="glass" className="directory-map-card">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                ...
                <p className="caption">
                  Google Maps base map and controlled directory markers stay in
                  sync.
                </p>
              </div>
            </div>
