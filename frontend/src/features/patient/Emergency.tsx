...
export const Emergency = () => {
  ...
  const [contactForSms, setContactForSms] = useState<{
    name: string;
    phone: string;
  } | null>(null);
  ...
  return (
    <div className="container" style={{ padding: 0, maxWidth: "760px" }}>
        ...
        <div className="flex items-center gap-3">
          <div
            style={{
              ...
              height: 52,
              borderRadius: "16px",
              background: "rgba(187, 44, 44, 0.12)",
              display: "flex",
              alignItems: "center",
          ...
          <div>
            ...
            <p className="caption" style={{ margin: "4px 0 0" }}>
              Choose an action yourself. LifeLink does not call emergency
              services, send messages, or share your location automatically.
            </p>
          </div>
        </div>
      </header>
      ...
      <div className="flex-col gap-4">
        <Card variant="emergency" style={{ padding: "var(--spacing-5)" }}>
          <div className="flex-col gap-3">
            ...
            <p style={{ margin: 0 }}>
              For an immediate emergency in India, you can open your device
              dialer for the unified emergency number{" "}
              <strong>{AMBULANCE_EMERGENCY_NUMBER}</strong>. Your device will
              ask you to place the call.
            </p>
            <Button
              variant="secondary"
              onClick={() => setIsAmbulanceConfirmOpen(true)}
            >
        ...
        <Card variant="glass" style={{ padding: "var(--spacing-5)" }}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              ...
              <p className="caption" style={{ margin: "3px 0 0" }}>
                Preparing a message opens your phone’s SMS composer. Review it
          ...
          {profileQuery.isLoading && (
            <p className="caption" style={{ margin: 0 }}>
              Loading your recorded emergency contacts…
            </p>
          )}
          {!profileQuery.isLoading && contacts.length === 0 && (
            <div className="flex-col gap-3">
              <p className="caption" style={{ margin: 0 }}>
                No emergency contacts are recorded in your Health Passport yet.
              </p>
              <Button
                variant="outline"
      ...
      <Popup
        isOpen={isAmbulanceConfirmOpen}
        onClose={() => setIsAmbulanceConfirmOpen(false)}
        title="Open emergency dialer"
        closeOnBackdrop={false}
      >
        <div className="flex-col gap-4">
          <p style={{ margin: 0 }}>
            This will open your device dialer with{" "}
            <strong>{AMBULANCE_EMERGENCY_NUMBER}</strong>. LifeLink will not
            place the call for you; you decide whether to continue in your phone
            app.
          </p>
          <div className="flex gap-3">
            <Button
              variant="outline"
              style={{ flex: 1 }}
              onClick={() => setIsAmbulanceConfirmOpen(false)}
            >
              Cancel
            </Button>
            <Button
              variant="secondary"
              style={{ flex: 1 }}
              onClick={openAmbulanceDialer}
            >
              <Phone size={16} /> Open dialer
            </Button>
          </div>
        </div>
      </Popup>
      ...
      <Popup
        isOpen={Boolean(contactForSms)}
        onClose={() => setContactForSms(null)}
        title={SMS_CONFIRMATION_TITLE}
        closeOnBackdrop={false}
      >
        <div className="flex-col gap-4">
          <p style={{ margin: 0 }}>
            This will open an SMS draft addressed to{" "}
            <strong>{contactForSms?.name}</strong>. LifeLink will not send it;
            you can review, edit, or cancel it in your messaging app.
          </p>
          <div className="flex gap-3">
            <Button
              variant="outline"
              style={{ flex: 1 }}
              onClick={() => setContactForSms(null)}
            >
              Cancel
            </Button>
            <Button
            ...
            >
              <MessageCircle size={16} /> Open SMS draft
            </Button>
          </div>
        </div>
      </Popup>
