...
export function sortByBrowserLocation<T extends MappableDirectoryEntry>(
  entries: readonly T[],
  ...
  const distanceSquared = (entry: T) => {
    ...
    const longitude =
      toRadians(entry.longitude - location.longitude) *
      Math.cos(toRadians(location.latitude));
    return latitude ** 2 + longitude ** 2;
  };

  return [...entries].sort(
    (left, right) => distanceSquared(left) - distanceSquared(right)
  );
}
