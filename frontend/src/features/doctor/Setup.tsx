...
export const DoctorSetup = () => {
  ...
  const bulkProvision = trpc.doctorAuth.provisionDirectory.useMutation({
    onSuccess: (result) => {
      ...
      setMessage(result.created.length ? "All remaining controlled clinician accounts have been created. Copy the one-time credentials now." : "All controlled clinician accounts already have separate credentials.");
      setProvisioningCode("");
    },
  ...
  return <main className="auth-page" aria-labelledby="doctor-setup-heading"><Card variant="glass" className="auth-card auth-card-wide">
    <header className="auth-card-header"><LifeLinkLogo className="lifelink-logo-auth" /><p className="caption">Controlled clinician accounts</p><h1 id="doctor-setup-heading">Provision clinician logins</h1><p>Create separate directory-aligned emails and passwords for controlled LifeLink specialist records. No verified clinician identity is created.</p></header>
    {message ? <div className="alert-panel auth-message" role="status">{message}</div> : null}
    <section style={{ display: "grid", gap: "var(--spacing-3)" }}>
      <div><h2 style={{ fontSize: "var(--text-h3)", marginBottom: "var(--spacing-1)" }}>All controlled specialists</h2><p className="caption">This creates the remaining separate accounts in one action. Existing accounts are skipped, so it is safe to use again.</p></div>
