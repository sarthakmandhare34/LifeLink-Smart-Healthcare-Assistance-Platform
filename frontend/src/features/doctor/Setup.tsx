...
export const DoctorSetup = () => {
  ...
  return <main className="auth-page" aria-labelledby="doctor-setup-heading"><Card variant="glass" className="auth-card auth-card-wide">
    ...
    <section style={{ display: "grid", gap: "var(--spacing-3)" }}>
      <div><h2 style={{ fontSize: "var(--text-h3)", marginBottom: "var(--spacing-1)" }}>Owner access</h2><p className="caption">Enter the private owner provisioning code to create missing accounts, view provisioned emails, replace one password, or securely refresh all account emails and passwords.</p></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "var(--spacing-2)" }}><Button type="button" variant="primary" disabled={!canUseOwnerTools || bulkProvision.isPending} onClick={() => bulkProvision.mutate({ provisioningCode })}>{bulkProvision.isPending ? "Creating accounts…" : "Create missing clinician logins"}</Button><Button type="button" variant="secondary" disabled={!canUseOwnerTools || loadOwnerAccounts.isPending} onClick={() => loadOwnerAccounts.mutate({ provisioningCode })}>{loadOwnerAccounts.isPending ? "Loading accounts…" : "View provisioned emails"}</Button><Button type="button" variant="outline" disabled={!canUseOwnerTools || refreshDirectoryCredentials.isPending} onClick={() => refreshDirectoryCredentials.mutate({ provisioningCode })}>{refreshDirectoryCredentials.isPending ? "Refreshing credentials…" : "Refresh all emails and passwords"}</Button></div>
    </section>
