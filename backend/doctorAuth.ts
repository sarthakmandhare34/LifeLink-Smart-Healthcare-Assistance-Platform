...
const resetInput = credentialInput.extend({
  provisioningCode: z.string().min(1).max(256),
});
...
function controlledClinicianEmail(
  doctor: (typeof mockDoctorDirectory)[number]
) {
  const local = `${doctor.railLine}-${doctor.specialty}-${doctor.station}`
    .replace(/[^a-z0-9]+/gi, ".")
    .replace(/^\.|\.$/g, "")
    .toLowerCase();
...
function matchesProvisioningCode(value: string) {
  const expected = ENV.clinicianAdministrationCode;
...
async function establishDoctorSession(
  ...
  if (!session)
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "Synthetic doctor session is not valid.",
    });
  const token = await sessionAuth.createSessionToken(openId, {
...
export const doctorAuthRouter = router({
  directory: publicProcedure.query(() =>
    mockDoctorDirectory.map(doctor => ({
      id: doctor.id,
      displayName: doctorDisplayName(doctor),
      specialty: doctor.specialty,
      locality: doctor.locality,
  ...
  provision: publicProcedure
    ...
    .mutation(async ({ input }) => {
      if (!matchesProvisioningCode(input.provisioningCode)) {
        throw new TRPCError({
          code: "UNAUTHORIZED",
          message: "The provisioning code is invalid.",
        });
      }
      ...
      if (!doctor)
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Selected controlled specialist was not found.",
        });
      ...
      if (!created)
        throw new TRPCError({
          code: "CONFLICT",
          message:
            "This clinician account or email already has credentials. Use the clinician sign-in page.",
        });
      return {
        doctorId: doctor.id,
        email: created.email,
  ...
  provisionDirectory: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      for (const doctor of mockDoctorDirectory) {
        ...
        const account = await createSyntheticDoctorCredential({
          doctor,
          email,
          passwordHash: await hashPatientPassword(password),
        });
        if (account)
          created.push({
            doctorId: doctor.id,
  ...
  refreshDirectoryCredentials: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      for (const doctor of mockDoctorDirectory) {
        ...
        const rotation = await refreshSyntheticDoctorCredentialByDoctorId({
          doctorId: doctor.id,
          email,
          passwordHash,
        });
        if (rotation === "email-conflict") {
          throw new TRPCError({
            code: "CONFLICT",
            message:
              "A refreshed clinician email conflicts with another account.",
          });
        }
        if (rotation === "not-found") {
          ...
          if (!created)
            throw new TRPCError({
              code: "CONFLICT",
              message: "A clinician account could not be refreshed safely.",
            });
        }
        refreshed.push({
          doctorId: doctor.id,
  ...
  replacePassword: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const password = `LL-${randomBytes(14).toString("base64url")}`;
      const updated = await updateSyntheticDoctorPasswordByEmail(
        email,
        await hashPatientPassword(password)
      );
      if (!updated)
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "No controlled clinician account uses that email.",
        });
  ...
  login: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const record = await getSyntheticDoctorCredentialByEmail(
        normalizedEmail(input.email)
      );
      const valid = record
        ? await verifyPatientPassword(
            input.password,
            record.credential.passwordHash
          )
        : false;
      const doctorId = record
        ? doctorIdFromSyntheticOpenId(record.user.openId)
        : null;
      if (
        !record ||
        !valid ||
        ...
        !doctorId ||
        record.credential.doctorId !== doctorId
      ) {
        throw new TRPCError({
          code: "UNAUTHORIZED",
          message: "Invalid clinician email or password.",
        });
      }
  ...
  resetPassword: publicProcedure
    ...
    .mutation(async ({ input }) => {
      if (!matchesProvisioningCode(input.provisioningCode)) {
        throw new TRPCError({
          code: "UNAUTHORIZED",
          message: "The controlled reset code is invalid.",
        });
      }
      const updated = await updateSyntheticDoctorPasswordByEmail(
        normalizedEmail(input.email),
        await hashPatientPassword(input.password)
      );
      if (!updated)
        throw new TRPCError({
          code: "NOT_FOUND",
  ...
  changePassword: doctorProcedure
    .input(changePasswordInput)
    .mutation(async ({ ctx, input }) => {
      const credential = await getSyntheticDoctorCredentialByUserId(
        ctx.user.id
      );
      ...
      const updated = await updateSyntheticDoctorPasswordByUserId(
        ctx.user.id,
        await hashPatientPassword(input.newPassword)
      );
      if (!updated)
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Controlled clinician credentials are unavailable.",
        });
      return { success: true } as const;
    }),
