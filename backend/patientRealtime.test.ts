...
describe("patient realtime isolation", () => {
  it("only delivers an event to the matching patient channel", () => {
    ...
    const stopFirst = subscribeToPatientEvents(1, event =>
      firstPatientEvents.push(event.id)
    );
    const stopSecond = subscribeToPatientEvents(2, event =>
      secondPatientEvents.push(event.id)
    );
  ...
  it("only delivers a doctor event to the matching controlled synthetic doctor channel", () => {
    ...
    const stopPrimary = subscribeToDoctorEvents(
      "mock-central-cardiology-csmt",
      event => primaryDoctorEvents.push(event.id)
    );
    const stopOther = subscribeToDoctorEvents(
      "mock-western-general-practice-churchgate",
      event => otherDoctorEvents.push(event.id)
    );
