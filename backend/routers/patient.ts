...
export const patientProfileRouter = router({
  ...
  emergencyContacts: router({
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const updated = await updateOwnedPatientEmergencyContact(
          ctx.user.id,
          input.id,
          input.values
        );
...
export const patientDashboardRouter = router({
  summary: protectedProcedure.query(({ ctx }) =>
    getPatientDashboard(ctx.user.id)
  ),
});
...
export const patientMedicineRouter = router({
  list: protectedProcedure.query(({ ctx }) =>
    listPatientMedicines(ctx.user.id)
  ),
  create: protectedProcedure
    .input(medicineInput)
    .mutation(async ({ ctx, input }) => {
  ...
  update: protectedProcedure
    .input(
      z.object({
        id: z.number().int().positive(),
        values: medicineInput.partial(),
      })
    )
    .mutation(async ({ ctx, input }) => {
      const updated = await updateOwnedPatientMedicine(
        ctx.user.id,
        input.id,
        input.values
      );
...
export const patientAppointmentRouter = router({
  ...
  request: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const id = await createPatientAppointment(
        ctx.user.id,
        doctor.id,
        input.scheduledAt,
        input.reason
  ...
  cancel: protectedProcedure
    .input(z.object({ id: z.number().int().positive() }))
    .mutation(async ({ ctx, input }) => {
      const cancelled = await cancelOwnedPatientAppointment(
        ctx.user.id,
        input.id
      );
      if (!cancelled)
...
export const patientDiscoveryRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(({ input }) => filterMockDoctorDirectory(input)),
});
