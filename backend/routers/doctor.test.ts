...
describe("doctor workspace authorization", () => {
  ...
  it("uses the signed synthetic doctor identity instead of a browser-supplied doctor identifier", async () => {
    ...
    const patients = await doctorWorkspaceRouter
      .createCaller(context("doctor"))
      .patients();

    expect(mocks.listDoctorAppointments).toHaveBeenCalledWith(
      "mock-central-cardiology-csmt"
    );
  ...
  it("keeps a second signed synthetic doctor on a separate appointment channel", async () => {
    ...
    secondDoctor.user.openId =
      "synthetic-doctor:mock-western-general-practice-churchgate";

    await doctorWorkspaceRouter.createCaller(secondDoctor).appointments.list();

    expect(mocks.listDoctorAppointments).toHaveBeenCalledWith(
      "mock-western-general-practice-churchgate"
    );
    expect(mocks.listDoctorAppointments).not.toHaveBeenCalledWith(
      "mock-central-cardiology-csmt"
    );
  });
  ...
  it("exposes a patient-provided booking reason only on the signed doctor’s assigned appointment list", async () => {
    mocks.listDoctorAppointments.mockResolvedValue([
      {
        ...
        scheduledAt: new Date("2026-08-25T10:00:00Z"),
        status: "Requested",
        reason: "Review submitted symptom assessment",
        createdAt: new Date(),
        patientId: 9,
        patientName: "Patient record",
      },
    ...
    const appointments = await doctorWorkspaceRouter
      .createCaller(context("doctor"))
      .appointments.list();
  ...
  it("rejects a patient session before any doctor data helper is called", async () => {
    await expect(
      doctorWorkspaceRouter.createCaller(context("user")).patients()
  ...
  it("notifies only the affected patient after an authorized appointment decision", async () => {
    ...
    await expect(
      doctorWorkspaceRouter
        .createCaller(context("doctor"))
        .appointments.updateStatus({ id: 22, status: "Confirmed" })
    ).resolves.toEqual({ success: true });

    expect(mocks.updateDoctorAppointmentStatus).toHaveBeenCalledWith(
      "mock-central-cardiology-csmt",
      22,
      "Confirmed"
    );
    expect(mocks.createPatientEvent).toHaveBeenCalledWith(
      9,
      "APPOINTMENT_UPDATED",
      "22"
    );
  ...
  it("returns the minimum patient detail only through the signed doctor’s assigned appointment relationship", async () => {
    mocks.getDoctorAuthorizedPatientDetail.mockResolvedValue({
      patient: {
        id: 9,
        name: "Patient record",
      ...
      assessments: [
        {
          ...
          duration: "3 days",
          urgency: "MODERATE",
          reason: "Patient-provided context",
          specialty: "Pulmonology",
          guidance: "Seek review",
          createdAt: new Date(),
        },
      ],
    });
    ...
    await expect(
      doctorWorkspaceRouter
        .createCaller(context("doctor"))
        .patientDetail({ patientId: 9 })
    ).resolves.toMatchObject({ patient: { id: 9 } });
    expect(mocks.getDoctorAuthorizedPatientDetail).toHaveBeenCalledWith(
      "mock-central-cardiology-csmt",
      9
    );
  });
    ...
    await expect(
      doctorWorkspaceRouter
        ...
        .prescriptions.create({
          patientId: 9,
          clinicalNotes: "Review completed",
          items: [
            {
              name: "Example medicine",
              dosage: "1 tablet",
              instructions: "Follow the documented care plan",
            },
          ],
        })
    ).resolves.toEqual({ id: 91, status: "UNSIGNED / CONTROLLED WORKSPACE" });

    expect(mocks.createDoctorAuthorizedPrescription).toHaveBeenCalledWith(
